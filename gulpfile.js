...
var project 	 = 'wimachtendienk', // Project name, used for build zip.
	url          = 'https://wimachtendienk.com', // Local Development URL for BrowserSync. Change as-needed.
	...
	var gulp     	 = require('gulp'),
		...
		function sftpOpts(path) {
		    return {
		        host: "mckernan.in",
		        user: "mckernanin",
		        remotePath: "/srv/users/mckernanin/apps/wimachtendienk/public/wp-content/themes/wimachtendienk-theme" + (path || "")
		    };
		}
/**
 * Browser Sync
...
// Add the newer pipe to pass through newer images only
	return 	gulp.src(['./assets/img/raw/**/*.svg'])
				.pipe(newer('./assets/img/'))
				.pipe(rimraf({ force: true }))
				.pipe(svgmin({
						plugins: [{
 ...
 */
 ...
 gulp.task('upload', function () {
   // return 	gulp.src([build+'/**/', './.jshintrc','./.bowerrc','./.gitignore' ])
   return 	gulp.src(['**/*', '!**/*.scss', '!*.json', '!**/gulpfile.js', '!**/*.md', '!node_modules/**', '!**/.*'])
				.pipe(sftp(sftpOpts()));
 });
 ...
 */
 ...
 // Watch Task
 gulp.task('default', ['styles', 'vendorsJs', 'scriptsJs', 'images', 'svgmin', 'browser-sync'], function () {
 ...
 // No Watch Task
 gulp.task('no-watch', ['styles', 'vendorsJs', 'scriptsJs', 'images', 'svgmin']);
